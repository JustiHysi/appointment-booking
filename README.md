...
- **AI:** OpenRouter (OpenAI-compatible API) with GPT-4o-mini and `json_schema` structured output
...
- OpenRouter API key ([openrouter.ai](https://openrouter.ai) — pay-as-you-go, covers GPT-4o-mini and other models)
...
### Configure OpenRouter (for AI intake analysis)
...
│   ├── ai.ts                       # OpenRouter structured-output action (medical triage)
...
- **AI triage**: OpenRouter (GPT-4o-mini) with `json_schema` strict mode. Returns suggested specialty, urgency level, clinical summary, possible conditions, recommended tests, and flags
...
- **Structured output enforced** via OpenAI's `response_format: json_schema` (strict mode) — the model cannot return free text or malformed JSON
